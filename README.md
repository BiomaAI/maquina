...
forks. Foundry: Night Shift is commandable from its first snapshot. Its 95 checked
snapshots let one operator run two production orders, pipeline work, coordinate
collection and dispatch, encounter output backpressure, cancel safely, and
finish with 0, 10, or 20 liters delivered. Full-production branches require nine or ten player decisions. The original dispatch/departure conflict and
proved failure/repair path remain available. Every snapshot, assessment,
receipt, event, and outcome is generated by Lean.

The command deck presents compatible plans, object inspection, collision-aware
labels, camera controls, pausable and single-step resolution, and a history of
explored branches. The player can compare completed runs while the renderer
continues to execute no game rules.

Operation Veiled Accord adds imperfect-information strategy. Public claims,
verified coalition evidence, account-level escrow, information sets, hidden
partner commitments, simultaneous sealed reveals, cooperation, exploitation,
and betrayal are exported through protocol v4. Candidate availability and
redacted explanations factor only through the commander's observation, so the
browser cannot use authoritative assessment as a secret-state oracle.
...
- an Operation Nightglass game proving heterogeneous game-owned composition,
  ...
  costly, exposed, and aborted outcomes; and
- an Operation Veiled Accord game proving actor-safe command-surface
  noninterference, information-set strategy consistency, audience-scoped
...
- project actor-scoped observations and assess advertised candidate actions
  through the same authoritative transition boundary;
- construct visible command surfaces only from declared observations, group
...
2. Complete authorization and capability-binding policy, then broaden declared
   goals and multi-actor analysis beyond bounded game-owned command graphs.
