...
def catalog : ShowcaseCatalog where
  ...
  entries :=
    [{ id := "foundry-refuel-lifecycle"
       ...
       artifact := "generated/foundry-refuel-lifecycle.v1.json" },
     { id := "foundry-active-presence"
       ...
       artifact := "generated/foundry-active-presence.v1.json" }]
...
def writeAll (directory : System.FilePath) : IO Unit := do
  ...
  writeJson (directory / "catalog.v1.json") catalog
  let artifacts := Maquina.Games.Foundry.Showcase.artifacts
  match artifacts with
  | [refuel, presence] =>
      writeJson (directory / "foundry-refuel-lifecycle.v1.json") refuel
      writeJson (directory / "foundry-active-presence.v1.json") presence
